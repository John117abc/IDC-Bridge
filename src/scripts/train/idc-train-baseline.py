...
from agents.idc_agent_baseline import DiscreteIDCAgentBaseline
...
def _diag_init_step(config, agent, builder, ego_indices, episode_path_indices,
    ...
    state_tensor = torch.from_numpy(np.stack(states[:2])).to(agent.device)
    with torch.no_grad():
...
def train(config):
    ...
    agent = DiscreteIDCAgentBaseline(env, config, config.device, builder, ego_indices)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--config', type=str, default='/workspace/idc/src/configs/base_baseline.yaml',
                        help='YAML 配置文件路径')
